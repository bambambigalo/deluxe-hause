...
const toHTML = cardHome => `
<a class="card" href="./pages/project_gallery.html">
    <div class="img_card">
        <img src="${cardHome.img}" alt="">
    </div>
    <p class="top_text_card">${cardHome.title}</p>
    <p class="bot_text_card">${cardHome.text}</p>
</a>
`

function cardHome () {
    const html = arrCardsHome.map(toHTML).join('');
