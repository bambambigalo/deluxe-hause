...
const toHTML = cardCommercial => `
<a class="card" href="./pages/project_gallery.html">
    <div class="img_card">
        <img src="${cardCommercial.img}" alt="">
    </div>
    <p class="top_text_card">${cardCommercial.title}</p>
    <p class="bot_text_card">${cardCommercial.text}</p>
</a>
`

function cardCommercial () {
    const html = arrCerdsCommercial.map(toHTML).join('');
